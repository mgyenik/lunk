...
pub async fn download_model(
    ...
    let resp = req.send().await.map_err(LunkError::Http)?;
    ...
    while let Some(chunk) = stream.next().await {
        let chunk = chunk.map_err(LunkError::Http)?;
    ...
    let valid = tokio::task::spawn_blocking(move || verify_sha256(&sha_path, &expected))
        .await
        .map_err(|e| LunkError::Other(format!("sha256 task failed: {e}")))??;

    if !valid {
        // Delete corrupted file
        let _ = tokio::fs::remove_file(&part_path).await;
        return Err(LunkError::Other("SHA256 verification failed".into()));
    }
