//! Adobe Glyph List: maps glyph names to Unicode code points.
//!
//! Generated from lopdf's glyph name data. Sorted for binary search.
//! Also implements the uni/u prefix algorithm per the AGL spec.

/// Look up a glyph name and return its Unicode code point(s).
/// Handles:
/// 1. Direct AGL table lookup
/// 2. `uniXXXX` prefix (groups of 4 hex digits)
/// 3. `uXXXX` prefix (4-6 hex digits, single code point)
/// 4. Period suffix stripping (e.g., `A.swash` -> `A`)
/// 5. Underscore splitting (e.g., `T_h` -> `T` + `h`)
pub(crate) fn glyph_to_unicode(name: &[u8]) -> Option<Vec<u16>> {
    // Strip period suffix
    ...
    // Check for underscore-separated components
    if name.contains(&b'_') {
        let mut result = Vec::new();
        for component in name.split(|&b| b == b'_') {
            if let Some(codes) = lookup_single(component) {
                result.extend_from_slice(&codes);
            } else {
                return None; // Any failed component fails the whole thing
            }
        }
...
fn lookup_single(name: &[u8]) -> Option<Vec<u16>> {
    // 1. AGL table lookup
    if let Ok(idx) = GLYPH_LIST.binary_search_by_key(&name, |&(n, _)| n.as_bytes()) {
        return Some(vec![GLYPH_LIST[idx].1]);
    }

    let name_str = std::str::from_utf8(name).ok()?;

    // 2. uni prefix: uniXXXX (groups of 4 uppercase hex)
    if let Some(hex) = name_str.strip_prefix("uni")
        && !hex.is_empty() && hex.len() % 4 == 0 && hex.chars().all(|c| c.is_ascii_hexdigit() && !c.is_ascii_lowercase())
    {
            ...
            return if codes.is_empty() { None } else { Some(codes) };
    }

    // 3. u prefix: uXXXX to uXXXXXX (4-6 uppercase hex digits)
    if let Some(hex) = name_str.strip_prefix("u")
        && (4..=6).contains(&hex.len()) && hex.chars().all(|c| c.is_ascii_hexdigit() && !c.is_ascii_lowercase())
    {
        let val = u32::from_str_radix(hex, 16).ok()?;
        if val <= 0xD7FF || (0xE000..=0x10FFFF).contains(&val) {
            if val <= 0xFFFF {
...
/// Sorted Adobe Glyph List (4495 entries).
static GLYPH_LIST: &[(&str, u16)] = &[
    ("A", 0x0041),
    ("AE", 0x00c6),
    ("AEacute", 0x01fc),
    ("AEmacron", 0x01e2),
