...
fn get_backend() -> Result<&'static LlamaBackend> {
    ...
    let guard = BACKEND.lock().map_err(|e| LunkError::Llm(format!("lock: {e}")))?;
    if guard.is_some() {
        ...
        Err(LunkError::Llm("backend init failed".into()))
    }
}
...
impl LlmEngine {
    ...
    pub fn load_model(&self, path: &Path, catalog_id: &str) -> Result<()> {
        ...
        let model = LlamaModel::load_from_file(backend, path, &params)
            .map_err(|e| LunkError::Llm(format!("model load: {e}")))?;

        let mut guard = self
            .inner
            .model
            .write()
            .map_err(|e| LunkError::Llm(format!("lock poisoned: {e}")))?;
        ...
        F: FnMut(&str),
    {
        ...
        let loaded = guard
            .as_ref()
            .ok_or_else(|| LunkError::Llm("no model loaded".into()))?;
        ...
        let tokens = model
            ...
            .map_err(|e| LunkError::Llm(format!("tokenize: {e}")))?;
        ...
        let mut ctx = model
            ...
            .map_err(|e| LunkError::Llm(format!("context: {e}")))?;
        ...
        for (i, &token) in tokens.iter().enumerate() {
            batch
                ...
                .map_err(|e| LunkError::Llm(format!("batch add: {e}")))?;
        }
        ...
        for _ in 0..params.max_tokens {
            ...
            ctx.decode(&mut batch)
                .map_err(|e| LunkError::Llm(format!("decode: {e}")))?;
        }
