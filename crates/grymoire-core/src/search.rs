...
pub fn search(
    ...
    if query.trim().is_empty() {
        return Err(GrymoireError::InvalidInput("search query cannot be empty".to_string()));
    }

    if query.len() > 1000 {
        return Err(GrymoireError::InvalidInput("search query too long (max 1000 chars)".to_string()));
    }
    ...
    for row in rows {
        ...
        let created_at = chrono::DateTime::parse_from_rfc3339(&row.created_at)
            .map_err(|e| GrymoireError::Other(format!("invalid timestamp: {e}")))?
