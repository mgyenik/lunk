...
/// ALPN protocol identifier for grymoire sync.
pub const SYNC_ALPN: &[u8] = b"grymoire/sync/1";
...
impl SyncNode {
    ...
    pub async fn new(data_dir: &Path, db: DbPool) -> Result<Self> {
        ...
        let endpoint = Endpoint::builder(presets::N0)
            ...
            .bind()
            .await
            .map_err(|e| GrymoireError::Transport(format!("failed to bind endpoint: {e}")))?;
    ...
    pub async fn sync_with_peer(&self, peer_id: &str) -> Result<SyncReport> {
        let endpoint_id: EndpointId = peer_id
            .parse()
            .map_err(|e| GrymoireError::Transport(format!("invalid peer id: {e}")))?;
        ...
        let conn: Connection = self
            ...
            .await
            .map_err(|e| GrymoireError::Transport(format!("connect failed: {e}")))?;

        let (mut send, mut recv) = conn
            .open_bi()
            .await
            .map_err(|e| GrymoireError::Transport(format!("open stream: {e}")))?;
        ...
        let (their_peer_version, their_changesets, their_tombstones, their_db_version) =
            match reply {
                ...
                _ => return Err(GrymoireError::Sync("expected Reply message".into())),
            };
        ...
        send.finish()
            .map_err(|e| GrymoireError::Transport(format!("finish send: {e}")))?;
    ...
    pub async fn shutdown(self) -> Result<()> {
        self.router
            .shutdown()
            .await
            .map_err(|e| GrymoireError::Transport(format!("shutdown: {e}")))?;
        Ok(())
    }
}
...
async fn handle_incoming_sync(
...
) -> std::result::Result<(), GrymoireError> {
    ...
    let (mut send, mut recv) = connection
        .accept_bi()
        .await
        .map_err(|e| GrymoireError::Transport(format!("accept stream: {e}")))?;
    ...
    let their_peer_version = match init {
        ...
        _ => return Err(GrymoireError::Sync("expected Init message".into())),
    };
    ...
    let (their_changesets, their_tombstones, their_db_version) = match payload {
        ...
        _ => return Err(GrymoireError::Sync("expected Payload message".into())),
    };
...
fn load_or_create_secret_key(data_dir: &Path) -> Result<SecretKey> {
    ...
    if key_path.exists() {
        ...
        let arr: [u8; 32] = bytes.try_into().map_err(|_| {
            GrymoireError::Config("invalid secret key file (wrong length)".to_string())
        })?;
