...
impl EmbeddingModel {
    ...
    pub fn new(cache_dir: Option<&std::path::Path>) -> Result<Self> {
        ...
        let inner = fastembed::TextEmbedding::try_new(opts)
            .map_err(|e| GrymoireError::Other(format!("embedding model init: {e}")))?;
    ...
    pub fn from_dir(model_dir: &std::path::Path) -> Result<Self> {
        let read = |name: &str| -> Result<Vec<u8>> {
            std::fs::read(model_dir.join(name)).map_err(|e| {
                GrymoireError::Other(format!("missing model file {name}: {e}"))
            })
        };
        ...
        .map_err(|e| GrymoireError::Other(format!("embedding model init from dir: {e}")))?;
    ...
    pub fn embed_text(&self, text: &str) -> Result<Vec<f32>> {
        ...
        let results = self
            ...
            .map_err(|e| GrymoireError::Other(format!("embedding failed: {e}")))?;

        results
            .into_iter()
            .next()
            .ok_or_else(|| GrymoireError::Other("no embedding returned".into()))
    }
}
...
pub fn find_similar(
    ...
    let query_blob: Vec<u8> = conn
        ...
        .map_err(|_| GrymoireError::Other("entry has no embedding".into()))?;
    ...
    for (id_str, score) in scored {
        let uuid: Uuid = id_str.parse().map_err(|e| GrymoireError::Other(format!("{e}")))?;
