...
#[derive(Parser)]
#[command(name = "grymoire", about = "Personal link indexing, archiving, and search")]
struct Cli {
...
async fn main() {
    tracing_subscriber::fmt()
        .with_env_filter(EnvFilter::from_default_env().add_directive("grymoire=info".parse().unwrap()))
        .with_target(false)
        .init();
    ...
    let result = match cli.command {
        ...
        None => {
            eprintln!("No command provided. Run `grymoire --help` for usage.");
            Ok(())
        }
    };
