...
use grymoire_core::config::{self, Config};
use grymoire_core::db;
use grymoire_core::errors::{GrymoireError, Result};
use grymoire_core::models::*;
use grymoire_core::repo;
use grymoire_core::schema;
use grymoire_core::search;
use grymoire_core::sync;
...
pub async fn save_url(url: &str, tags: &[String]) -> Result<()> {
    ...
    if text.is_empty() {
        return Err(GrymoireError::Other("could not extract any text from the page".to_string()));
    }
...
pub async fn import_pdf(path: &str, title: Option<&str>, tags: &[String]) -> Result<()> {
    ...
    if !file_path.exists() {
        return Err(GrymoireError::InvalidInput(format!("file not found: {path}")));
    }
    ...
    if pages.is_empty() {
        return Err(GrymoireError::Other("could not extract any text from PDF".to_string()));
    }
...
pub fn tag_entry(id: &str, tags: &[String], remove: bool) -> Result<()> {
    ...
    let uuid = uuid::Uuid::parse_str(id)
        .map_err(|e| GrymoireError::InvalidInput(format!("invalid id: {e}")))?;
...
pub async fn serve(port: u16) -> Result<()> {
    ...
    let embedding_model = grymoire_core::embeddings::EmbeddingModel::new(None)?;
    let llm_engine = grymoire_core::llm_engine::LlmEngine::new()?;
    let state = grymoire_server::state::AppState { db: pool, embedding_model, llm_engine, sync_node: None };
    let router = grymoire_server::build_router(state);

    let addr = format!("127.0.0.1:{port}");
    println!("Grymoire API server listening on http://{addr}");

    let listener = tokio::net::TcpListener::bind(&addr).await
        .map_err(|e| GrymoireError::Other(format!("failed to bind {addr}: {e}")))?;

    axum::serve(listener, router).await
        .map_err(|e| GrymoireError::Other(format!("server error: {e}")))?;
...
pub fn install_native_messaging(extension_id: &str, browser: &str) -> Result<()> {
    ...
    let binary_path = std::env::current_exe()
        .map_err(|e| GrymoireError::Other(format!("could not determine binary path: {e}")))?;

    let manifest = serde_json::json!({
        "name": "com.grymoire.app",
        "description": "Grymoire link archiving system",
    ...
    let manifest_path = host_dir.join("com.grymoire.app.json");
...
pub fn backfill_all() -> Result<()> {
    use grymoire_core::{chunks, embeddings, keywords};
...
pub fn llm_retitle(model_id: &str, bad_only: bool, filter_ids: &[String]) -> Result<()> {
    use grymoire_core::{llm_catalog, llm_engine, llm_models, llm_titles};
    use rusqlite::params;
    use std::time::Instant;
    ...
    let entry = llm_catalog::get_catalog_entry(model_id)
        .ok_or_else(|| GrymoireError::Other(format!("unknown model: {model_id}")))?;
    ...
    for (id, old_title, _content_type, extracted_text) in &rows {
        ...
        if bad_only && !grymoire_core::pdf::is_generic_title(old_title) {
            skipped += 1;
            continue;
        }
...
pub fn transfer(from: &str) -> Result<()> {
    ...
    if !source_path.exists() {
        return Err(GrymoireError::InvalidInput(format!(
    ...
    if source_path == dest_path {
        return Err(GrymoireError::InvalidInput(
...
fn extract_pdf_pages(data: &[u8]) -> Result<Vec<(i32, String)>> {
    let pages = grymoire_core::pdf::extract_pages(data);
    if pages.is_empty() {
        return Err(GrymoireError::Other("could not extract any text from PDF".to_string()));
    }
    Ok(pages)
...
pub async fn sync_trigger() -> Result<()> {
    ...
    if results.is_empty() {
        println!("No peers configured. Use `grymoire sync add <PEER_ID>` to add peers.");
    }
