...
use grymoire_core::db::{with_db, DbPool};
use grymoire_core::embeddings::EmbeddingModel;
use grymoire_core::llm_engine::{LlmEngine, SamplingParams};
...
pub async fn send_chat_message(
    ...
    tokio::task::spawn_blocking(move || {
        ...
        // Retrieve context
        let template = grymoire_core::llm_titles::active_chat_template(&engine);
        let rag_ctx = with_db(&db, |conn| {
            rag::prepare_rag_context(
                ...
                &rag_history,
                &message,
                template.unwrap_or(grymoire_core::llm_catalog::ChatTemplate::ChatML),
            )
        })
