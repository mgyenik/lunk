...
use grymoire_server::state::AppState;
use tauri::Manager;
...
pub fn run() {
    tracing_subscriber::fmt()
        .with_env_filter(
            tracing_subscriber::EnvFilter::try_from_default_env()
                .unwrap_or_else(|_| "info,grymoire=debug".parse().unwrap()),
        )
        .init();
    ...
    // In dev mode: files are at crates/grymoire-app/models/all-MiniLM-L6-v2/
    ...
    let embedding_model = {
        ...
        let candidates = [
            ...
            exe_dir.join("../lib/grymoire-app/models"),              // Linux AppImage/deb
            std::path::PathBuf::from("models/all-MiniLM-L6-v2"),// dev mode (cwd = crates/grymoire-app)
        ];
        ...
        if model_dir.join("model_quantized.onnx").exists() {
            ...
            grymoire_core::embeddings::EmbeddingModel::from_dir(&model_dir)
                .expect("failed to load bundled embedding model")
        } else {
            tracing::info!("downloading embedding model (first run)");
            grymoire_core::embeddings::EmbeddingModel::new(None)
    ...
    if !config.llm.active_model.is_empty()
        && let Some(entry) = grymoire_core::llm_catalog::get_catalog_entry(&config.llm.active_model)
        && let Ok(path) = grymoire_core::llm_models::model_path(entry)
        && path.exists()
    {
    ...
    tauri::Builder::default()
        ...
        .setup(move |app| {
            // System tray
            let show = MenuItem::with_id(app, "show", "Show Grymoire", true, None::<&str>)?;
            ...
            tauri::async_runtime::spawn(async move {
                ...
                tokio::spawn(async move {
                    use grymoire_core::db::with_db;
                    match with_db(&backfill_pool, |conn| {
                        let emb = grymoire_core::embeddings::embed_all_missing(conn, &backfill_model)?;
                        let kw = grymoire_core::keywords::extract_all_missing(conn)?;
                        let chunks = grymoire_core::chunks::chunk_all_missing(conn, &backfill_model)?;
                        Ok((emb, kw, chunks))
                    }) {
                        Ok((emb, kw, chunks)) => {
