...
impl Dictionary {
    ...
    pub fn get_font_encoding<'a>(&'a self, doc: &'a Document) -> Result<Encoding<'a>> {
        ...
        let encoding_obj = self.get_deref(b"Encoding", doc);

        // Try direct Name first (e.g. /WinAnsiEncoding)
        match encoding_obj.and_then(Object::as_name) {
            Ok(b"StandardEncoding") => return Ok(Encoding::OneByteEncoding(&encodings::STANDARD_ENCODING)),
            Ok(b"MacRomanEncoding") => return Ok(Encoding::OneByteEncoding(&encodings::MAC_ROMAN_ENCODING)),
            Ok(b"MacExpertEncoding") => return Ok(Encoding::OneByteEncoding(&encodings::MAC_EXPERT_ENCODING)),
            Ok(b"WinAnsiEncoding") => return Ok(Encoding::OneByteEncoding(&encodings::WIN_ANSI_ENCODING)),
            Ok(b"PDFDocEncoding") => {
                log::warn!("PDFDocEncoding is not a valid character encoding for a font");
                return Ok(Encoding::OneByteEncoding(&encodings::PDF_DOC_ENCODING));
            }
            ...
            Ok(name) => return Ok(Encoding::SimpleEncoding(name)),
            Err(_) => {}
        }

        // Try encoding dictionary with BaseEncoding (e.g. { /Type /Encoding /BaseEncoding /WinAnsiEncoding /Differences [...] })
        if let Ok(dict) = self.get_deref(b"Encoding", doc).and_then(Object::as_dict) {
            if let Ok(base) = dict.get(b"BaseEncoding").and_then(Object::as_name) {
                match base {
                    b"StandardEncoding" => return Ok(Encoding::OneByteEncoding(&encodings::STANDARD_ENCODING)),
                    b"MacRomanEncoding" => return Ok(Encoding::OneByteEncoding(&encodings::MAC_ROMAN_ENCODING)),
                    b"WinAnsiEncoding" => return Ok(Encoding::OneByteEncoding(&encodings::WIN_ANSI_ENCODING)),
                    b"MacExpertEncoding" => return Ok(Encoding::OneByteEncoding(&encodings::MAC_EXPERT_ENCODING)),
                    name => return Ok(Encoding::SimpleEncoding(name)),
                }
            }
        }

        // Fallback: try ToUnicode CMap
        warn!(
            "Could not parse the encoding for font: {self:#?}\nTrying to retrieve ToUnicode."
        );
